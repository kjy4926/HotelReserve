...
public class HotelPageUtil {
	...
	public int pageRequest(String keyword, Pageable pageable) {
		int count = Long.valueOf(hotelService.getHotelCount(keyword)).intValue();//총 컬럼개수
		int num = pageable.getPageNumber()/10;//현재페이지/10
		System.out.println(count);
		if((pageable.getPageNumber() + 1)%10 == 0) {//마지막 페이지(>>), 0페이지부터 시작이라 1을 더함
			this.setMaxPage(count/10 - 1);//마지막 데이터 개수가 0으로 끝나면 끝
		}else {
			this.setMaxPage(count/10);//마지막 데이터 개수가 0으로 안끝나면 한페이지 더 만들기
		}
		
//		this.setMaxPage(count-1);//데이터 1개전용 코드
